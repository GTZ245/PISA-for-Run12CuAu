...
void pisaToDST(
  ...
  //int run_number = 434912 //pAu
  int run_number = 372586 //CuAu
  //int run_number = 428212 //pp
)
{
  ...
  */
  ...
  vtx_sim->UseXY( true ); // this is the default
  vtx_sim->OverwriteBBC( true );  // this is the default
  vtx_sim->ZVertexSigma( 2.75 );
  vtx_sim->XVertexSigma( 0.0 );
  vtx_sim->YVertexSigma( 0.0 );
  ...
  #else
  VtxT0Simreco* vtx_sim = new VtxT0Simreco();
  vtx_sim->UseVtx( VtxT0Simreco::PISA );
  vtx_sim->ZVertexSigma(2.75);
  vtx_sim->XVertexSigma( 0.0 );
  vtx_sim->YVertexSigma( 0.0 );
  vtx_sim->T0Sigma(0.00);
  vtx_sim->UseXY( true ); // this is the default
  se->registerSubsystem( vtx_sim );

  // for old libraries TMutExtVtx must have its source set manually                                 
